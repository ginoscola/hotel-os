...
def test_week_start_legge_da_app_config(test_engine, TestSession):
    ...
    db = TestSession()
    try:
        result = _leggi_week_start(db=db)
    finally:
        db.close()
    assert result == 1


def test_week_start_fallback_se_chiave_assente(TestSession):
...
def test_week_start_cache_viene_resettata(test_engine, TestSession):
