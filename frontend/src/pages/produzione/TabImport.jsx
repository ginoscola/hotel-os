...
import { isAdmin, fmtD, thSt, tdSt } from '../../utils/produzioneHelpers'
...
export default function TabImport({ onImportato }) {
  ...
  const eliminaImport = async (imp) => {
    const msg = imp.is_test
      ? 'Eliminare questa sessione di import di test?'
      : `Eliminare l'import reale "${imp.nome_file}" (${imp.n_righe_valide} righe)? Verranno rimossi anche tutti i dati collegati.`
        ...
        {caricamento ? (
          ...
          <>
            ...
            <p style={{ color: '#475569', margin: '0.5rem 0 0' }}>
              Trascina il file StatisticheProduzione.xlsx esportato da Welcome PMS
              <br />
              <span style={{ fontSize: '0.82rem', color: '#94a3b8' }}>
                Registro analitico riga-per-riga (diverso da listaConti.xlsx) — oppure clicca per selezionare
              </span>
            </p>
          </>
        )}
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.85rem' }}>
          <thead>
            <tr style={{ background: '#1e3a5f', color: '#fff' }}>
              {['Data import', 'File', 'Periodo', 'Strutture', 'Righe valide', 'Riassetto', 'Escluse', ''].map(h => (
            ...
            {storico.map((imp, idx) => (
              <tr key={imp.id} style={{ background: idx % 2 === 0 ? '#fff' : '#f8fafc' }}>
                <td style={{ ...tdSt, textAlign: 'left', color: '#64748b' }}>
                  ...
                  {imp.is_test && <span style={{ marginLeft: 6, background: '#fef3c7', color: '#92400e', padding: '1px 6px', borderRadius: 4, fontSize: '0.7rem', fontWeight: 600 }}>TEST</span>}
                </td>
                ...
                <td style={tdSt}>{imp.n_righe_valide}</td>
                <td style={tdSt}>{imp.n_righe_riassetto}</td>
                <td style={{ ...tdSt, color: imp.n_righe_escluse > 0 ? '#92400e' : '#94a3b8' }}>{imp.n_righe_escluse}</td>
                <td style={tdSt}>
                  {isAdmin() && (
                    <button onClick={() => eliminaImport(imp)}
